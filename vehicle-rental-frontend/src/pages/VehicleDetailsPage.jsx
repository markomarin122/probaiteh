...
const VehicleDetailsPage = () => {
    ...
    const fetchSelectData = async () => {
        try {
            ...
            const [catRes, branchRes] = await Promise.all([
                axios.get('http://localhost:8000/api/stats'), // Often contains some info or we can use specific routes if they exist
            ]);
    ...
    const fetchClients = async (searchTerm = '') => {
        ...
        try {
            const res = await axios.get(`http://localhost:8000/api/users?search=${searchTerm}`, {
    ...
    const handleUpdateVehicle = async (e) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const res = await axios.put(`http://localhost:8000/api/vehicles/${id}`, editedData, {
                headers: { Authorization: `Bearer ${token}` }
            });
