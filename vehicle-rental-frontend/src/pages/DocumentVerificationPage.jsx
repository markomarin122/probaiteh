...
const DocumentVerificationPage = () => {
    ...
    const fetchDocuments = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:8000/api/documents/all?status=${filter}`, {
    ...
    const handleApprove = async (id) => {
        const token = localStorage.getItem('token');
        try {
            await axios.post(`http://localhost:8000/api/documents/${id}/approve`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleReject = async (id) => {
        const token = localStorage.getItem('token');
        try {
            await axios.post(`http://localhost:8000/api/documents/${id}/reject`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
                    ...
                    documents.map((doc) => (
                        <div key={doc.id} className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-shadow">
                            ...
                            <div className="mb-4">
                                ...
                                <a
                                    href={`http://localhost:8000/${doc.putanja}`}
                                    target="_blank"
